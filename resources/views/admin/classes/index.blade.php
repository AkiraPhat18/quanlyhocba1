...
<div class=""> <!-- class="table-responsive" để có thanh kéo -->
	<div class="form-group m-form__group row align-items-center">
		...
		<div class="col-md-3" style="padding-left: 100px">
			<a href="{{route('class.add')}}" class="btn btn-info m-btn m-btn--custom m-btn--icon m-btn--air">
					<span>
	...
	<table class="table table-bordered table-hover">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					{{$element->classes_name}}
				</td>
				<td>
					{{$element->classes_student_numbers}}
				</td>
				<td>
					@if($element->classes_active)
				...
				<td data-field="" class="m-datatable__cell">
					<span style="overflow: visible; position: relative; width: 120px;">						
						<a href="{{route('class.edit', ['id' => $element->id])}}" class="m-portlet__nav-link btn m-btn m-btn--hover-info m-btn--icon m-btn--icon-only m-btn--pill" title="Cập nhật">								
							<i class="la la-edit"></i>							
						</a>
												
						<a href="{{route('class.remove', ['id' => $element->id])}}" data-value="42" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill m-btn--delete" title="Xóa">
